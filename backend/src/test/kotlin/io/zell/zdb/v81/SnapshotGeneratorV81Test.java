...
import static io.zell.zdb.TestUtils.createZeebeContainerBefore85;
...
class SnapshotGeneratorV81Test {

  private static final Logger LOGGER = LoggerFactory.getLogger(SnapshotGeneratorV81Test.class);
  private static final DockerImageName DOCKER_IMAGE =
      DockerImageName.parse("camunda/zeebe:8.1.18");
  private static final File TEMP_DIR = TestUtils.newTmpFolder(SnapshotGeneratorV81Test.class);
  private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

  // Relative to backend/ module root — Maven sets CWD to the module during tests
  private static final Path SNAPSHOT_TARGET = Path.of("src/test/resources/zeebe-states/v8.1");
  private static final Path SNAPSHOT_ZIP =
      Path.of("src/test/resources/zeebe-states/v8.1.zip");
  ...
  static void startContainerAndCreateContent() {
    zeebeContainer =
        createZeebeContainerBefore85(DOCKER_IMAGE, TEMP_DIR.getPath(), LOGGER);
  ...
  void generateSnapshot() throws Exception {
    ...
    // then: write metadata.json with all keys needed by Version81GoldenTest
    final var metadata =
    ...
    // Zip relative to src/test/resources so entries carry the zeebe-states/v8.1/ prefix,
    // matching the path the golden test resolves after unzipping (snapshotDir = tempRoot.resolve("zeebe-states/v8.1"))
