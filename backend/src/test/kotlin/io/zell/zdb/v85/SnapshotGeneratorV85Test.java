...
class SnapshotGeneratorV85Test {

  private static final Logger LOGGER = LoggerFactory.getLogger(SnapshotGeneratorV85Test.class);
  private static final DockerImageName DOCKER_IMAGE =
      DockerImageName.parse("camunda/zeebe:8.5.0");
  private static final File TEMP_DIR = TestUtils.newTmpFolder(SnapshotGeneratorV85Test.class);
  private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

  // Relative to backend/ module root — Maven sets CWD to the module during tests
  private static final Path SNAPSHOT_TARGET = Path.of("src/test/resources/zeebe-states/v8.5");
  private static final Path SNAPSHOT_ZIP =
      Path.of("src/test/resources/zeebe-states/v8.5.zip");
  ...
  void generateSnapshot() throws Exception {
    ...
    // then: write metadata.json with all keys needed by Version85GoldenTest
    final var metadata =
    ...
    // Zip relative to src/test/resources so entries carry the zeebe-states/v8.5/ prefix,
    // matching the path the golden test resolves after unzipping (snapshotDir = tempRoot.resolve("zeebe-states/v8.5"))
